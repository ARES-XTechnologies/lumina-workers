...
def encode_image(
    ...
    """Encode a result as base64, falling back to JPEG if PNG is too large.

    Runpod's job-done endpoint rejects payloads over ~10MB. A colourised photo
    at full camera resolution can cross that as a lossless PNG, so this tries
    PNG first and only drops to JPEG — at decreasing quality — if it has to.
    Never triggers when `keep_alpha` is set, since JPEG has no alpha channel.
    """
    ...
    for quality in (92, 85, 75, 60):
        ...
        rgb.convert("RGB").save(
            buffer, format="JPEG", quality=quality, subsampling=1, optimize=True
        )
